...
void LinearLayer::allocateWeightsMemory() {
	cudaError_t error = cudaMallocManaged(&weights, x_dim * y_dim * sizeof(float));
...
void LinearLayer::initializeWeightsRandomly() {
	for (int x = 0; x < x_dim; x++) {
		for (int y = 0; y < y_dim; y++) {
			weights[y * x_dim + x] = (static_cast<float>(rand()) / RAND_MAX) * weights_init_threshold;
		}
	}
}
...
float* LinearLayer::forward(float* A, int A_x_dim, int A_y_dim) {
	...
	linear_layer_forward<<<block_size, num_of_blocks>>>(A, weights, Z,
														A_x_dim, A_y_dim,
														x_dim, y_dim);
	cudaDeviceSynchronize();
	nn_utils::throwIfDeviceErrorsOccurred("Cannot perform forward prop.");
